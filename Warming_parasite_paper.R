...
pfu$Phage <- factor(pfu$Phage, levels=c("PEV2","LUZ19","phage_14_1"), labels=c("PEV2","LUZ19","14-1"))
...
qPCR_data_single <- subset(qPCR_data, Treatment==-c('3P','PL','P14','L14'))
qPCR_data_single <- qPCR_data[which(qPCR_data$Treatment==c('P','L','phage14')),] 

options(scipen=999)

qPCR_data_single$Phage <- factor(qPCR_data_single$Phage, levels=c("PEV2","LUZ19","phage14"), labels=c("PEV2","LUZ19","14-1"))
...
## Figure S1. Temperature has a phage-specific impact on phage lytic activity
...
## Figure S2. Bacterial growth in absence of phage is similar between temperatures
...
nophageT5 <- nophage[(nophage$time_point=="T5"),]

model <- lme(OD ~ Temp, random= ~1|Batch, data= nophageT5)
summary(model)
anova(model)
...
CFU_nophage <- CFU_wphage[(CFU_wphage$Phage=="No phage"),]
...
PEV2_decayrate <- ggplot(datlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  #stat_smooth_func(geom="text",method="glm",hjust=0,vjust=1,parse=TRUE) +
  #geom_smooth(method="glm", formula = y ~ x,
  #method.args = list(family = poisson)) +
  #geom_ribbon(data=PEV2_37C_50,aes(ymin=lowerfit, ymax=upperfit), alpha = 0.3) + 
  ...
  labs(colour="Temperature (C)")+
  scale_color_manual(values = c("#ffeda0","#feb24c","#fc4e2a"))+
  theme(legend.title = element_text(colour="black", size=10,face="bold"),legend.text = element_text(size=12))+
...
LUZ19_decayrate <- ggplot(datlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  geom_ribbon( aes(ymin = lowerfit, ymax = upperfit, group = Temp), alpha = .15) +
  geom_line( aes(y = fit), size = 0.8)+
  geom_jitter(width = 0.2) +
  #scale_y_continuous(trans='log10', limits = c(10^3,10^5)) +
...
phage14_decay <- ggplot(datlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  #stat_smooth_func(geom="text",method="glm",hjust=0,vjust=1,parse=TRUE) +
  #geom_smooth(method="glm", formula = y ~ x,
  #method.args = list(family = poisson)) +
  #geom_ribbon(data=PEV2_37C_50,aes(ymin=lowerfit, ymax=upperfit), alpha = 0.3) + 
...
adsorption_assay$Dif_PFU_ml<- as.numeric(adsorption_assay$Dif_PFU_ml)
...
PEV2_37C_50 <- predict(model, data.frame(Time = new_x, Temp = factor(rep("37", length(new_y)))),
                       type = "response", se.fit=T)
PEV2_37C_50$CI <- PEV2_37C_50$se.fit * 1.96
PEV2_37C_50$Time <- new_x
PEV2_37C_50 <- data.frame(PEV2_37C_50)
...
PEV2_40C_50 <- predict(model, data.frame(Time = new_x, Temp = factor(rep("40", length(new_y)))),
                       type = "response", se.fit=T)
PEV2_40C_50$CI <- PEV2_40C_50$se.fit * 1.96
PEV2_40C_50$Time <- new_x
PEV2_40C_50 <- data.frame(PEV2_40C_50)
...
PEV2_42C_50 <- predict(model, data.frame(Time = new_x, Temp = factor(rep("42", length(new_y)))),
                       type = "response", se.fit=T)
PEV2_42C_50$CI <- PEV2_42C_50$se.fit * 1.96
PEV2_42C_50$Time <- new_x
PEV2_42C_50 <- data.frame(PEV2_42C_50)
...
PEV2_adsorption <- ggplot(datlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  #stat_smooth_func(geom="text",method="glm",hjust=0,vjust=1,parse=TRUE) +
  #geom_smooth(method="glm", formula = y ~ x,
  #method.args = list(family = poisson)) +
  #geom_ribbon(data=PEV2_37C_50,aes(ymin=lowerfit, ymax=upperfit), alpha = 0.3) + 
...
LUZ19_adsorption <- ggplot(datlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  #stat_smooth_func(geom="text",method="glm",hjust=0,vjust=1,parse=TRUE) +
  #geom_smooth(method="glm", formula = y ~ x,
  #method.args = list(family = poisson)) +
  #geom_ribbon(data=PEV2_37C_50,aes(ymin=lowerfit, ymax=upperfit), alpha = 0.3) + 
...
phage14_adsorption <- ggplot(alldatlm, aes(Time, PFU_ML,colour = as.factor(Temp))) +
  #stat_smooth_func(geom="text",method="glm",hjust=0,vjust=1,parse=TRUE) +
  #geom_smooth(method="glm", formula = y ~ x,
  #method.args = list(family = poisson)) +
  #geom_ribbon(data=PEV2_37C_50,aes(ymin=lowerfit, ymax=upperfit), alpha = 0.3) + 
...
radar <- read.csv("Radar_charts_correct.csv",fileEncoding="UTF-8-BOM")

pev2_radar <- radar[(radar$Phage=="PEV2"),]
pev2_radar <- subset(pev2_radar, select = c("Temperature","Decay_prop","Recip_adsorption_time_prop","T5_T0_pfu_prop","log_T5_T0_CFU_prop_norm") )
colnames(pev2_radar) <- c("Temperature","Survivorship" , "Host attachment" , "Reproductive capacity" , "Virulence")

luz19_radar <- radar[(radar$Phage=="LUZ19"),]
luz19_radar <- subset(luz19_radar, select = c("Temperature","Decay_prop","Recip_adsorption_time_prop","T5_T0_pfu_prop","log_T5_T0_CFU_prop_norm") )
colnames(luz19_radar) <- c("Temperature","Survivorship" , "Host attachment" , "Reproductive capacity" , "Virulence")

phage14_radar <- radar[(radar$Phage=="phage14"),]
phage14_radar <- subset(phage14_radar, select = c("Temperature","Decay_prop","Recip_adsorption_time_prop","T5_T0_pfu_prop","log_T5_T0_CFU_prop_norm") )
colnames(phage14_radar) <- c("Temperature","Survivorship" , "Host attachment" , "Reproductive capacity" , "Virulence")

ggradar(pev2_radar, values.radar = c(0, 0.5, 1),
...
ggradar(luz19_radar, values.radar = c(0, 0.5, 1),
        background.circle.colour = "grey90",
        axis.line.colour = "gray60",
        plot.extent.x.sf =1.2,
        gridline.min.colour = "gray60",
...
ggradar(phage14_radar, values.radar = c(0, 0.5, 1),
        background.circle.colour = "grey90",
        axis.line.colour = "gray60",
        plot.extent.x.sf =1.2,
        gridline.min.colour = "gray60",
...
ggplot(radar,aes(x=Log_average_ratio_T2_and_T0_CFU,y=Log_average_ratio_T2_and_T0_PFU, group=Phage))+ 
  geom_point(aes(shape = Phage, fill = Temperature), col="black",size=3)+
  ...
  geom_errorbar(aes(ymin=Log_average_ratio_T2_and_T0_PFU-Log_average_ratio_T2_and_T0_PFU_SE, ymax=Log_average_ratio_T2_and_T0_PFU+Log_average_ratio_T2_and_T0_PFU_SE), width=.01,col="grey",
                position=position_dodge(.01)) +
  geom_errorbar(aes(xmin=Log_average_ratio_T2_and_T0_CFU-Log_average_ratio_T2_and_T0_CFU_SE, xmax=Log_average_ratio_T2_and_T0_CFU + Log_average_ratio_T2_and_T0_CFU_SE), height=.03,col="grey",
                position=position_dodge(.01)) +
...
# Figure 3A - Phage competition at 37C

# PEV2
pev2_qPCR_late_37 <- PEV2_qPCR[(PEV2_qPCR$Time=="5" & PEV2_qPCR$Temp==37),]
pev2_qPCR_late_37$Competitor <- factor(pev2_qPCR_late_37$Competitor, levels=c("No_competitor","LUZ19", "phage14","3P"), labels=c("No competitor","LUZ19","14-1","3-phage"))

model <- lme(DNA_conc ~ Competitor, random= ~1|Batch, data= pev2_qPCR_late_37, method = "REML", weights = varIdent(form = ~1|Competitor))
plot(model)
anova(model)
...
# LUZ19
luz19_qPCR_late_37 <- LUZ19_qPCR[(LUZ19_qPCR$Time=="5" & LUZ19_qPCR$Temp==37),]
luz19_qPCR_late_37$Treatment <- factor(luz19_qPCR_late_37$Treatment, levels=c("luz19", "PL", "L14","3P"), labels=c("LUZ19", "P + L","L + 14-1", "3-phage"))
luz19_qPCR_late_37$Competitor <- factor(luz19_qPCR_late_37$Competitor, levels=c("No_competitor","PEV2", "phage14","3P"), labels=c("No competitor","PEV2","14-1","3-phage"))

# LUZ19
model <- lme(DNA_conc ~ Competitor, random= ~1|Batch, data= luz19_qPCR_late_37, method = "REML", weights = varIdent(form = ~1|Competitor))
plot(model)
anova(model)
emmeans(model, pairwise ~ Competitor)

# phage 14-1
phage14_qPCR_late_37 <- phage14_qPCR[(phage14_qPCR$Time=="5" & phage14_qPCR$Temp==37),]
phage14_qPCR_late_37$Treatment <- factor(phage14_qPCR_late_37$Treatment, levels=c("phage14", "P14", "L14","3P"), labels=c("14-1", "P + 14-1","L + 14-1", "3-phage"))
phage14_qPCR_late_37$Competitor <- factor(phage14_qPCR_late_37$Competitor, levels=c("No_competitor","PEV2", "LUZ19","3P"), labels=c("No competitor","PEV2","LUZ19","3-phage"))

# 14-1
model <- lme(DNA_conc ~ Competitor, random= ~1|Batch, data= phage14_qPCR_late_37, method = "REML", weights = varIdent(form = ~1|Competitor))
plot(model)
anova(model)
emmeans(model, pairwise ~ Competitor)


pev2_comp_T5_37 <- ggplot(data=pev2_qPCR_late_37, aes(Competitor, Fold_change_DNAconc_to_zero, fill = Temp))  + 
...
luz19_comp_T5_37 <- ggplot(data=luz19_qPCR_late_37, aes(Competitor, Fold_change_DNAconc_to_zero, fill = Temp))  + 
  #geom_violin(alpha=0.5, position = position_dodge(width = .75),size=1,color=NA)+
  geom_boxplot(width=0.5,notch = FALSE,  outlier.size = -1, color="black",lwd=1, alpha = 0.7,show.legend = F, varwidth=FALSE,position = position_dodge(width = .75))+
  ggbeeswarm::geom_quasirandom(shape = 21,size=2, dodge.width = .75, color="black",alpha=.5,show.legend = F)+
  ylab("Relative phage DNA concentration") +
  ...
  scale_y_continuous(trans='log10', limits = c(100,22000)) +
  annotation_logticks(sides="l")+
  geom_signif(y_position = c(2.6,2.8,3), xmin = c(1,1,1), 
              xmax = c(2,3,4), annotation = c("***","*","**"),
              tip_length = 0.05)+
  theme_bw()+
  geom_smooth(method = "lm")+
  scale_fill_manual(values = "#ffeda0") +
...
phage14_comp_T5_37 <- ggplot(data=phage14_qPCR_late_37, aes(Competitor, Fold_change_DNAconc_to_zero, fill = Temp))  + 
  #geom_violin(alpha=0.5, position = position_dodge(width = .75),size=1,color=NA)+
  geom_boxplot(width=0.5,notch = FALSE,  outlier.size = -1, color="black",lwd=1, alpha = 0.7,show.legend = F, varwidth=FALSE,position = position_dodge(width = .75))+
  ggbeeswarm::geom_quasirandom(shape = 21,size=2, dodge.width = .75, color="black",alpha=.5,show.legend = F)+
  ylab("Relative phage DNA concentration") +
  ...
  scale_y_continuous(trans='log10', limits = c(100,22000)) +
  annotation_logticks(sides="l")+
  geom_signif(y_position = c(4.2,4), xmin = c(1,1), 
              xmax = c(2,4), annotation = c("**","**"),textsize=5,
              tip_length = 0.03)+
  theme_bw()+
  geom_smooth(method = "lm")+
  scale_fill_manual(values = "#ffeda0") +
...
pev2_comp_T5_37+luz19_comp_T5_37+ phage14_comp_T5_37




# Figure 3B - Phage competition at 37C and 40C

### 37C v 40C comparison
...
pev2_3740_tempcomp <- ggplot(data=pev2_qPCR_late_3740, aes(Competitor, Fold_change_DNAconc_to_zero, fill = Temp))  + 
  ...
  annotation_logticks(sides="l")+
  geom_signif(y_position = 2.8, xmin = 2.75, 
              xmax = 3.25, annotation = "*",
              tip_length = 0.05)+
  theme_bw()+
  geom_smooth(method = "lm")+
...
phage14_3740_tempcomp <- ggplot(data=phage14_qPCR_late_3740, aes(Competitor, Fold_change_DNAconc_to_zero, fill = Temp))  + 
  ...
  scale_y_continuous(trans='log10', limits = c(100,10000)) +
  annotation_logticks(sides="l")+
  geom_signif(y_position = c(3.3,3.1), xmin = c(1.75,3.75), 
              xmax = c(2.25,4.25), annotation = c("***","***"),
              tip_length = 0.05)+
  theme_bw()+
  labs(fill="Temperature (\u00B0C)")+
  geom_smooth(method = "lm")+
...
average_res$Phage <- factor(average_res$Phage, levels=c("PEV2", "LUZ19", "14_1"), labels=c(expression(phi*"PEV2"), expression(phi*"LUZ19"), expression(phi*14-1)))

ggplot(average_res,aes(x=Average_restricting_opposite,y=Average_restricted, group = Phage))+ 
  geom_point(aes(shape = Phage, fill = Temp), col="black",size=3)+
  geom_path(aes(x = Average_restricting_opposite, y = Average_restricted, group = Phage), 
            arrow = arrow(length = unit(0.35, "cm")), col="darkgrey")+
  labs(y="Resistance to competitors",x="Suppression of competitors")+
  theme_bw()+
  ylim(0,1.25)+
  xlim(-0.1,1)+
  scale_fill_manual(values = c("#ffeda0","#feb24c","#fc4e2a"))+
...
## Figure S5 - Phage competition at 37C after 2h
...
## Figure S6 - Phage competition at 42C
